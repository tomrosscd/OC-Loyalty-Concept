...
export default function Home(){
 ...
 return <main className={`hotel ${entered?'entered':''} ${closed?'doors-closed':''} ${phase}`}>
  <header className="header"><div className="header-start"><a className="shop-link" href={SHOP_URL}><Arrow dir="left" size={13}/><span>BACK TO <span className="desktop-word">THE </span>SHOP</span></a><a className="outcast" href={SHOP_URL} aria-label="Outcast home"><img src={asset('outcast-white.svg')} alt="Outcast" width="150" height="20"/></a></div><span className="header-caption">THE HEARTBREAK HOTEL <span>·</span> OUTCAST LOYALTY</span><button className="services-link" onClick={()=>openPanel('directory')}><Sparkle/> GUEST SERVICES</button></header>
  <section className={`elevator floor-${tier.id}`} aria-label="Heartbreak Hotel elevator" onTouchStart={e=>{touch.current={x:e.touches[0].clientX,y:e.touches[0].clientY}}} onTouchEnd={e=>{if(!entered||panel||perk||join||busy.current||(e.target as HTMLElement).closest('button'))return;const dy=touch.current.y-e.changedTouches[0].clientY,dx=touch.current.x-e.changedTouches[0].clientX;if(Math.abs(dy)>65&&Math.abs(dy)>Math.abs(dx)*1.4)void travel(floor+Math.sign(dy))}}>
   ...
   <div className="portal" style={{'--scene-ratio':1376/768,'--img-ratio':848/1264,'--max-ratio':848/(1264-mobileCropTop[floor])} as CSSProperties}>
    ...
    {entered&&<><div className="scene-top"><span>THE HEARTBREAK HOTEL</span><span>0{floor+1} / 04</span></div><div className="explore-hint"><Plus size={11}/> EXPLORE YOUR PRIVILEGES</div><div className="swipe-hint" aria-hidden="true">SWIPE FOR MORE FLOORS <Arrow dir="up-down" size={11}/></div></>}
    </div>
  ...
  <footer><span>STAY A LITTLE. WANT A LOT.</span><span>HEARTBREAK LOOKS GOOD ON YOU.</span><button onClick={()=>openPanel('directory')}>HOTEL DIRECTORY <Arrow size={9}/></button></footer>
  <Sheet open={!!perk||!!panel} onOpenChange={o=>{if(!o){setPerk(null);setPanel(null)}}}><SheetContent side={mobile?'bottom':'right'} className={`hotel-sheet ${panel==='directory'?'directory-sheet':''}`} onCloseAutoFocus={returnFocus}>
   ...
   {perk&&<><div className="benefit-value">{perk.value}</div><span className="benefit-tier">{tier.name.toUpperCase()} PRIVILEGE</span><button className="primary" onClick={()=>{setPerk(null);setPanel('tier')}}>EXPLORE ALL PRIVILEGES <Arrow/></button></>}
